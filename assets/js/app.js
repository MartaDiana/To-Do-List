...
/** Pasang atribut tema di <html data-theme="..."> */
function applyTheme(theme){
  document.documentElement.setAttribute('data-theme', theme === 'dark' ? 'dark' : 'light');
}

/** Atur tampilan tombol tema di header */
function initTheme(){
  const t = localStorage.getItem(THEME_KEY) || 'light';
  applyTheme(t);
  const btn = $('#themeBtn');
  btn.innerHTML = (t === 'dark' ? '<i class="bi bi-brightness-high"></i> Terang'
...
function pipeline(){
  ...
  // 1) mulai dari semua data
  let list = [...state.tasks];

  // 2) filter status (kecuali “semua”)
